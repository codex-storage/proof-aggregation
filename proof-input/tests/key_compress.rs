...
const EXPECTED_COMPRESS_OUTPUTS_U64: [[u64; 4]; 4] = [
...
mod key_compress_tests {
    ...
    pub fn test_key_compress(){
        ...
        for (key, &expected_u64s) in EXPECTED_COMPRESS_OUTPUTS_U64.iter().enumerate() {
...
#[cfg(test)]
mod key_compress_circuit_tests {
    use super::*;

    /// tests the in-circuit key_compress with concrete cases
    #[test]
    pub fn test_key_compress_circuit(){
        // `HashOut` for inputs
        ...
        for (i, &expected_u64s) in EXPECTED_COMPRESS_OUTPUTS_U64.iter().enumerate() {
            let expected: [F; 4] = expected_u64s.map(|x| F::from_canonical_u64(x));
